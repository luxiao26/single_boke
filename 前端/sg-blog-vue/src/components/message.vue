...
<template>
    <div class="tmsgBox"  ref="tmsgBox">
        ...
        <div class="tmsg-comments"  ref="listDom">
            <a href="#" class="tmsg-comments-tip">评论区</a>
            <div class="tmsg-commentshow">
                <ul class="tmsg-commentlist">
                    <li class="tmsg-c-item" v-for="(item,index) in commentList" :key="'common'+index">
                        <article class="">
                            <header>
                                <img v-if="item.avater!=null" :src="item.avater">
                                <img v-if="item.avater==null" src="static/img/tou.jpg">
                                <div class="i-name">
                                    {{item.username}} <span v-if="item.createBy=='1'"><el-tag type="success">管理员</el-tag></span>
                                </div>
                                <!-- <div class="i-class">
                                    {{item.label}}
                                </div> -->
                        ...
                        <ul v-show="item.children" class="tmsg-commentlist" style="padding-left:60px;">
                            <li class="tmsg-c-item" v-for="(citem,cindex) in item.children" :key="'citem'+cindex">
                                <article class="">
                                    <header>
                                            <img v-if="item.avater!=null" :src="citem.avater">
                                            <img v-if="item.avater==null" src="static/img/tou.jpg">
                                            <div class="i-name">
                                                {{citem.username}}
                                                <span v-if="citem.createBy=='1'"><el-tag type="success">管理员</el-tag></span>
                                                <span>回复</span> {{citem.toCommentUserName}}
                                                <span v-if="citem.toCommentUserId=='1'"><el-tag type="success">管理员</el-tag></span>
                                            </div>
                                            <div class="i-time">
          ...
          sendMsg:function(){//留言
              ...
              if(that.textarea){
                ...
                if (!localStorage.getItem('userInfo')) {
                    return this.$message.error('需要登录后才能发送评论')
                }
                let userInfoObj = JSON.parse(userInfoStr);
                    sendComment(that.type,that.aid,that.rootId,that.toCommentId,that.toCommentUserId,that.textarea,userInfoObj.id).then((response)=>{
                        that.textarea = '';
                        that.rootId = -1;
                        that.toCommentId = -1;
